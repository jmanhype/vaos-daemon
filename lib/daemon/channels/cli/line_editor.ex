defmodule Daemon.Channels.CLI.LineEditor do
  @moduledoc """
  ...
  defp toggle_task_display(tty) do
    try do
      sessions =
        try do
          :ets.match(:daemon_settings, {{:"$1", :task_display_visible}, :"$2"})
        rescue
          _ -> []
        end
      ...
      case sessions do
        [[sid, current] | _] ->
          new_val = !current
          :ets.insert(:daemon_settings, {{sid, :task_display_visible}, new_val})

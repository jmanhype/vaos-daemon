defmodule Daemon.Providers.Registry do
  @moduledoc """
  ...
  ## Public API (backward-compatible)
      ...
      # Basic usage
      Daemon.Providers.Registry.chat(messages)

      # With options
      Daemon.Providers.Registry.chat(messages, provider: :groq, temperature: 0.5)

      # List all registered providers
      Daemon.Providers.Registry.list_providers()

      # Get info about a specific provider
      Daemon.Providers.Registry.provider_info(:groq)
  ...
  The module must implement the `Daemon.Providers.Behaviour`.
  ...
  defp ollama_reachable? do
    Daemon.Providers.Ollama.reachable?()
  end
  ...
  defp call_with_fallback(provider, module, messages, opts) do
    case Daemon.Providers.ConcurrencyLimiter.with_limit(fn ->
      ...
      {:error, reason} = err ->
        HealthChecker.record_failure(provider, reason)
        fallback_chain = Application.get_env(:daemon, :fallback_chain, [])
        ...
        case matched do
          ...
          nil ->
            ...
            case get_ollama_context(model) do
              {:ok, ctx} -> ctx
              _ -> Application.get_env(:daemon, :max_context_tokens, 128_000)
            end
        end
  ...
  def context_window(_), do: Application.get_env(:daemon, :max_context_tokens, 128_000)

defmodule Daemon.Commands.Session do
  @moduledoc """
  ...
  def cmd_sessions(_arg, _session_id) do
    sessions = Daemon.Agent.Memory.list_sessions()
      ...
      if sessions == [] do
        ...
        body =
          ...
          |> Enum.map_join("\n", fn s ->
            ...
            last = Daemon.Commands.Info.format_timestamp(s[:last_active])
            hint = s[:topic_hint] || ""
  ...
  def cmd_resume(arg, _session_id) do
    ...
    if target == "" do
      ...
      case Daemon.Agent.Memory.resume_session(target) do
        {:ok, messages} ->
  ...
  defp cmd_history_search(query, channel_filter) do
    ...
    _ ->
      results = Daemon.Agent.Memory.search_messages(query, limit: 20)

...
Audience: operators running Daemon in production or staging environments.
...
Authorization: Bearer <jwt>   (only when DAEMON_REQUIRE_AUTH=true)
Accept: text/event-stream
```

Each SSE message is a JSON-encoded `%Daemon.Events.Event{}` struct:
...
Daemon emits standard Telemetry events for all major operations. Attach handlers
...
:telemetry.attach_many(
  ...
  [
    [:daemon, :llm, :request, :stop],
    [:daemon, :tool, :execute, :stop],
    [:daemon, :session, :start],
    [:daemon, :session, :stop],
    [:daemon, :budget, :exceeded]
  ],
...
2. `[:daemon, :budget, :exceeded]` telemetry event is emitted
...
`Daemon.Telemetry.Metrics` tracks runtime statistics and writes a
JSON snapshot to `~/.daemon/metrics.json` every 5 minutes.
...
| Provider p99 > 30s | Analytics endpoint | Switch provider via `DAEMON_DEFAULT_PROVIDER` |
| DLQ size > 100 | `DLQ.size()` | Investigate handler failure; flush and fix |
| SQLite file > 1 GB | `~/.daemon/osa.db` size | Archive old sessions; see backup-recovery.md |

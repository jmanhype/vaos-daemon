defmodule Daemon.Commands.Model do
  @moduledoc """
  ...
  def active_model_for(provider) do
    ...
    case Application.get_env(:daemon, model_key) do
      nil ->
  ...
  defp cmd_model_list do
    registry = MiosaProviders.Registry
    current = Application.get_env(:daemon, :default_provider, :unknown)
  ...
  defp do_model_switch(provider, model_override, extra_warning \\ nil) do
    Application.put_env(:daemon, :default_provider, provider)

    if model_override do
      model_key = :"#{provider}_model"
      Application.put_env(:daemon, model_key, model_override)
    else
  ...
  defp cmd_ollama_models do
    ...
    current_model = Application.get_env(:daemon, :ollama_model, "detecting...")
  ...
  defp cmd_model_set_ollama_url(url) do
    if url == "" do
      current = Application.get_env(:daemon, :ollama_url, "http://localhost:11434")

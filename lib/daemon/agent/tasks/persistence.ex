defmodule Daemon.Agent.Tasks.Persistence do
  @moduledoc """
  Unified persistence layer for the Agent.Tasks subsystem.

  Provides two storage backends:
  - JSON files for Workflow (one file per workflow: ~/.daemon/workflows/{id}.json)
  - JSON files for Tracker tasks (per-session: ~/.daemon/sessions/{sid}/tasks.json)
  ...
  def workflows_dir do
    Application.get_env(:daemon, :workflows_dir, "~/.daemon/workflows")
    |> Path.expand()
  end

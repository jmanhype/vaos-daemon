...
## Daemon's Supervision Tree

Daemon's application supervisor starts these children in order using the
...
before Daemon accepts external requests.
...
├── [MiosaBudget.Treasury]   — opt-in: DAEMON_TREASURY_ENABLED=true
...
├── [Fleet.Supervisor]       — opt-in: DAEMON_FLEET_ENABLED=true
...
├── [Sandbox.Supervisor]     — opt-in: DAEMON_SANDBOX_ENABLED=true
├── [Integrations.Wallet]    — opt-in: DAEMON_WALLET_ENABLED=true
├── [System.Updater]         — opt-in: DAEMON_UPDATE_ENABLED=true
...
`DynamicSupervisor` allows Daemon to:
...
Supervisor.which_children(Daemon.Supervisors.Infrastructure)
...
[ets-and-persistent-term.md](./ets-and-persistent-term.md) to learn how Daemon

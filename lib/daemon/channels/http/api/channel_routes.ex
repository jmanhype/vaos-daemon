defmodule Daemon.Channels.HTTP.API.ChannelRoutes do
  @moduledoc """
  ...
  import Daemon.Channels.HTTP.API.WebhookVerify
  require Logger

  alias Daemon.Channels.Telegram
  alias Daemon.Channels.Discord
  alias Daemon.Channels.Slack
  alias Daemon.Channels.WhatsApp
  alias Daemon.Channels.Signal, as: SignalChannel
  alias Daemon.Channels.Matrix
  alias Daemon.Channels.Email, as: EmailChannel
  alias Daemon.Channels.QQ
  alias Daemon.Channels.DingTalk
  alias Daemon.Channels.Feishu
  ...
  post "/telegram/webhook" do
    secret = Application.get_env(:daemon, :telegram_webhook_secret)
  ...
  post "/whatsapp/webhook" do
    app_secret = Application.get_env(:daemon, :whatsapp_app_secret)
    raw_body = conn.assigns[:raw_body] || Jason.encode!(conn.body_params)
  ...
  post "/signal/webhook" do
    secret = Application.get_env(:daemon, :signal_webhook_secret)
    raw_body = conn.assigns[:raw_body] || Jason.encode!(conn.body_params)
  ...
  post "/email/inbound" do
    secret = Application.get_env(:daemon, :email_webhook_secret)

...
The current implementation slice is `vas-swarm-jji.6`:
...
- the next cut is to keep `retrieval_ops`-only `artifact_reference` runs local by suppressing external paper-search bleed without reintroducing topic-family routing
...
- `vas-swarm-jji.6` — active: keep retrieval-ops-only artifact/reference investigations local unless mixed-source retrieval is explicit

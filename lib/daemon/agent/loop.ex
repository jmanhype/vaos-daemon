defmodule Daemon.Agent.Loop do
  @moduledoc """
  ...
  alias Daemon.Agent.Loop.ToolExecutor
  alias Daemon.Agent.Loop.Guardrails
  alias Daemon.Agent.Loop.LLMClient
  alias Daemon.Agent.Loop.Checkpoint
  alias Daemon.Agent.Loop.GenreRouter

  alias Daemon.Intelligence.ConversationTracker
  alias Daemon.Intelligence.CommCoach
  alias Daemon.Intelligence.ContactDetector
  alias Daemon.Telemetry.Metrics

  defp max_iterations, do: Application.get_env(:daemon, :max_iterations, 30)
  defp auto_insights_interval, do: Application.get_env(:daemon, :auto_insights_interval, 10)
  defp max_response_tokens, do: Application.get_env(:daemon, :max_response_tokens, 8_192)
  ...
  def start_link(opts) do
    ...
    GenServer.start_link(__MODULE__, opts, name: {:via, Registry, {Daemon.SessionRegistry, session_id, user_id}})
  end
  ...
  def init(opts) do
    ...
    state = %__MODULE__{
      ...
      plan_mode_enabled: Application.get_env(:daemon, :plan_mode_enabled, false),
      permission_tier: Keyword.get(opts, :permission_tier, :full),
      working_dir: Keyword.get(opts, :working_dir) || Application.get_env(:daemon, :working_dir)
    }
    ...
    {strategy_mod, strategy_state} =
      case Strategy.resolve(strategy_context) do
        ...
        {:error, _} ->
          # Fallback to ReAct
          react = Daemon.Agent.Strategies.ReAct
  ...
  def handle_call({:process, message, opts}, _from, state) do
    ...
    Process.delete(:daemon_git_info_cache)
    Process.delete(:daemon_workspace_overview_cache)

    # Clear system message cache at start of each process call (Phase 4)
    Process.delete(:daemon_system_msg_cache)
    Process.put(:daemon_memory_version, 0)
    ...
    compacted = Daemon.Agent.Compactor.maybe_compact(state.messages) || state.messages
  ...
  @impl true
  def handle_call({:swap_provider, provider, model}, _from, state) do
    :ets.insert(:daemon_session_provider_overrides, {state.session_id, provider, model})
    {:reply, :ok, %{state | provider: provider, model: model}}
  end
    ...
    case result do
      ...
      {:ok, %{content: content, tool_calls: tool_calls} = resp} when is_list(tool_calls) ->
        ...
        if Enum.any?(tool_calls, fn tc -> tc.name == "memory_save" end) and
          ...
          Process.put(:daemon_memory_version, Process.get(:daemon_memory_version, 0) + 1)
        end
      ...
      {:error, reason} ->
        ...
        if context_overflow?(reason_str) and state.overflow_retries < 3 do
          ...
          compacted = Daemon.Agent.Compactor.maybe_compact(state.messages)
  ...
  defp cached_context(state) do
    cache_key =
      {state.plan_mode, state.session_id, Process.get(:daemon_memory_version, 0), state.channel}

    case Process.get(:daemon_system_msg_cache) do
      ...
      _ ->
        ...
        case full do
          %{messages: [system_msg | _]} when system_msg != nil ->
            Process.put(:daemon_system_msg_cache, {cache_key, system_msg})
            full
  ...
  defp via(session_id), do: {:via, Registry, {Daemon.SessionRegistry, session_id}}
  ...
  defp emit_context_pressure(state) do
    ...
    estimated = Daemon.Agent.Compactor.estimate_tokens(state.messages)
  ...
  defp estimate_tokens_for_introspection(state) do
    try do
      Daemon.Agent.Compactor.estimate_tokens(state.messages)
    rescue
      _ -> 0
    end
  end
  ...
  @survey_table :daemon_survey_answers

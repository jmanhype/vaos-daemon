defmodule Daemon.Tools.Builtins.Delegate do
  @moduledoc """
  ...
  def execute(%{"task" => task} = params) do
    ...
    agent_id = Daemon.Utils.ID.generate("dlg")
  ...
  defp resolve_model(tier) do
    case tier do
      :elite ->
        Application.get_env(:daemon, :elite_model) ||
          Application.get_env(:daemon, :anthropic_model)

      :specialist ->
        Application.get_env(:daemon, :specialist_model)

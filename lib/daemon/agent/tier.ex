defmodule Daemon.Agent.Tier do
  @moduledoc """
  ...
  def detect_ollama_tiers do
    ...
    case safe_list_ollama_models(url) do
      {:ok, models} when models != [] ->
        ...
        :persistent_term.put(:daemon_ollama_tiers, mapping)
      ...
      _ ->
        ...
        :persistent_term.put(:daemon_ollama_tiers, %{})
        {:error, :no_models}
    end
  end
  ...
  defp auto_model(provider) do
    ...
    Application.get_env(:daemon, key) ||
      Application.get_env(:daemon, :default_model, "claude-sonnet-4-6")
  end
  ...
  defp safe_get_ollama_tiers do
    try do
      :persistent_term.get(:daemon_ollama_tiers)
    rescue
      ArgumentError -> %{}
    end
  end
  ...
  def set_tier_override(tier, model) when tier in [:elite, :specialist, :utility] do
    ...
    :persistent_term.put(:daemon_tier_overrides, Map.put(overrides, tier, model))
    :ok
  end
  ...
  def clear_tier_override(tier) when tier in [:elite, :specialist, :utility] do
    overrides = get_tier_overrides()
    :persistent_term.put(:daemon_tier_overrides, Map.delete(overrides, tier))
    :ok
  end
  ...
  def get_tier_overrides do
    try do
      :persistent_term.get(:daemon_tier_overrides)
    rescue
      ArgumentError -> %{}
    end
  end

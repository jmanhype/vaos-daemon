defmodule Daemon.Tools.Registry do
  @moduledoc """
  ...
  2. SKILL.md files from ~/.daemon/skills/ (markdown-defined, parsed at boot)
  3. MCP server tools (auto-discovered from ~/.daemon/mcp.json)

  The registry maintains a goldrush-compiled :daemon_tool_dispatcher module
  ...
  defp skills_dir, do: Application.get_env(:daemon, :skills_dir, "~/.daemon/skills")
  ...
  def register_mcp_tools do
    tools = Daemon.MCP.Client.list_tools()
  ...
  @doc "Reload skills from disk (~/.daemon/skills/) and recompile the dispatcher."
  ...
  def active_skills_context do
    ...
    if map_size(skills) == 0 do
      ...
      skills_dir = Path.expand(Application.get_env(:daemon, :skills_dir, "~/.daemon/skills"))
  ...
  defp resolve_priv_skills_path do
    case :code.priv_dir(:daemon) do
      {:error, _} ->
        ...
        app_dir = Application.app_dir(:daemon)
  ...
  defp load_skills do
    ...
    # Load user skills from ~/.daemon/skills/ (higher priority — override priv skills)
  ...
  #
  # Compiles a goldrush module (:daemon_tool_dispatcher) that validates tool
  ...
  defp compile_dispatcher(builtin_tools, _skills) do
    if map_size(builtin_tools) > 0 do
      ...
      case :glc.compile(:daemon_tool_dispatcher, query) do
        {:ok, _} -> :ok
        error -> Logger.warning("Failed to compile :daemon_tool_dispatcher: #{inspect(error)}")
      end
    end
  rescue
    _ -> :ok

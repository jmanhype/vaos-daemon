defmodule Daemon.Channels.HTTP.API.AuthRoutes do
  @moduledoc """
  ...
  When `DAEMON_REQUIRE_AUTH` is true (or `DAEMON_SHARED_SECRET` is set), the
  ...
  defp verify_login_secret(conn) do
    require_auth = Application.get_env(:daemon, :require_auth, false)
    configured_secret = Application.get_env(:daemon, :shared_secret)
    ...
    if require_auth or not is_nil(configured_secret) do
      ...
      Logger.warning(
        "[AuthRoutes] Login without secret verification — " <>
          "set DAEMON_SHARED_SECRET or DAEMON_REQUIRE_AUTH=true to enable auth"
      )

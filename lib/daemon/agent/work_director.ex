defmodule Daemon.Agent.WorkDirector do
  ...
  defp finalize_branch(item, branch, repo_path) do
    ...
    case Git.current_branch(repo_path) do
      ...
      {:error, reason} ->
        Logger.error("[WorkDirector] Stage 3 ABORT: failed to get current branch: #{reason}")
  ...
  defp finalize_branch_impl(item, branch, repo_path) do
    ...
    case Git.changed_files("HEAD", repo_path) do
      {:ok, output} when byte_size(output) > 2 ->
        ...
        if files == [] do
          ...
          Git.add_files(files, repo_path)

          # Commit
          commit_msg = "feat: #{item.title}\n\nSource: #{item.source}\nPriority: #{item.base_priority}\n\nAutonomously generated by WorkDirector staged execution."
          case Git.commit(commit_msg, repo_path) do
            :ok ->
              # Push
              case Git.push("origin", branch, repo_path) do
                :ok ->
                  # Create draft PR
      ...
      _ ->
        ...
        case Git.status_porcelain(repo_path) do
          {:ok, output} when byte_size(output) > 2 ->
  ...
  defp verify_safety(branch) do
    ...
    case Git.changed_files("main...#{branch}", repo_path) do
      {:ok, output} ->
  ...
  defp delete_branch(branch) do
    ...
    Git.delete_local_branch(branch, repo_path)
    Git.delete_remote_branch("origin", branch, repo_path)
  end
        ...
        {category, content} =
          case result do
            {:ok, _output, branch} ->
              ...
              diff_stat = with {:ok, stat} <- Git.diff_stat("main...#{branch}", ["*.ex", "*.exs"], repo_path), do: stat
              diff_content = with {:ok, diff} <- Git.diff_with_files("main...#{branch}", ["*.ex", "*.exs"], repo_path), do: diff

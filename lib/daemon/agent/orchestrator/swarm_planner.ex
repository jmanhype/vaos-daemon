defmodule Daemon.Agent.Orchestrator.SwarmPlanner do
  @moduledoc """
  Decomposes complex tasks into subtasks and assigns agent roles for swarm execution.
  ...
  This module is separate from `Agent.Orchestrator.Decomposer`, which handles
  single-session multi-agent task decomposition. SwarmPlanner is specifically
  for swarm coordination with pattern selection (parallel, pipeline, debate, review).
  ...
  def decompose(task_description, opts \\ []) do
    ...
    Logger.info(
      "SwarmPlanner decomposing task (max_agents=#{max_agents}): #{String.slice(task_description, 0, 100)}..."
    )

    case call_llm(task_description, max_agents) do
      {:ok, plan} ->
        Logger.info(
          "SwarmPlanner produced plan: pattern=#{plan.pattern} agents=#{length(plan.agents)}"
        )

        plan

      {:error, reason} ->
        Logger.warning("SwarmPlanner LLM call failed (#{inspect(reason)}), using fallback plan")
  ...
  defp strip_markdown_fences(content),
    do: Daemon.Utils.Text.strip_markdown_fences(content)
    ...
    validated =
      ...
      |> Enum.flat_map(fn agent ->
        case validate_agent(agent) do
          ...
          {:error, reason} ->
            Logger.warning("SwarmPlanner: skipping invalid agent spec (#{reason}): #{inspect(agent)}")
            []
        end
      end)

defmodule Daemon.Commands.System do
  @moduledoc """
  ...
  def cmd_setup(_arg, _session_id) do
    Daemon.Onboarding.Channels.run()
  ...
  def cmd_logs(arg, _session_id) do
    ...
    log_file = Application.get_env(:daemon, :log_file, "log/dev.log")
  ...
  def cmd_completion(arg, _session_id) do
    ...
    commands = Daemon.Commands.list_commands() |> Enum.map(fn {name, _desc, _cat} -> name end)
  ...
  def cmd_docs(arg, _session_id) do
    ...
    docs = %{
      ...
      Config files:
        ~/.daemon/config.json  — Provider, API keys, machines
        ~/.daemon/.env         — Environment overrides
        .env (project root) — Project-specific overrides

      Provider priority: DAEMON_DEFAULT_PROVIDER > API key detection > ollama
  ...
  def cmd_update(_arg, _session_id) do
    current = Application.spec(:daemon, :vsn) |> to_string()
    ...
    result =
      case parse_create_args(arg) do
        {:ok, name, description, template} ->
          case Daemon.Commands.register(name, description, template) do
  ...
  def cmd_workflow(arg, _session_id) do
    cmd_name = Process.get(:daemon_current_cmd, "unknown")

    case Daemon.PromptLoader.get_command("workflow", cmd_name) do
      nil ->
  ...
  def cmd_prime(arg, _session_id) do
    cmd_name = Process.get(:daemon_current_cmd, "prime")

    case Daemon.PromptLoader.get_command("context", cmd_name) do
      nil ->
        loaded = Daemon.PromptLoader.list_command_prompts()
  ...
  def cmd_security(arg, _session_id) do
    cmd_name = Process.get(:daemon_current_cmd, "security-scan")

    case Daemon.PromptLoader.get_command("security", cmd_name) do
      nil ->
  ...
  def cmd_memory_cmd(arg, _session_id) do
    cmd_name = Process.get(:daemon_current_cmd, "mem-search")

    case Daemon.PromptLoader.get_command("memory", cmd_name) do
      nil ->
  ...
  def cmd_utility(arg, _session_id) do
    ...
    case Daemon.PromptLoader.get_command("utility", cmd_name) do
      nil ->
  ...
  @doc false
  def check_soul do
    identity = Daemon.Soul.identity()
    soul = Daemon.Soul.soul()
    ...
    cond do
      ...
      true -> {:warn, "Soul", "using defaults (no ~/.daemon/IDENTITY.md or SOUL.md)"}
    end
  end
  ...
  @doc false
  def check_memory do
    stats = Daemon.Agent.Memory.memory_stats()
  ...
  @doc false
  def check_cortex do
    stats = Daemon.Agent.Cortex.synthesis_stats()
  ...
  def check_scheduler do
    case Process.whereis(Daemon.Agent.Scheduler) do

defmodule Daemon.Sandbox.Docker do
  @moduledoc """
  ...
  - `:workspace`      — host path to mount as `/workspace` (default `~/.daemon/workspace`)
  ...
  def execute(command, opts \\ []) do
    workspace =
      Keyword.get(opts, :workspace, Path.expand("~/.daemon/workspace"))
      |> Path.expand()

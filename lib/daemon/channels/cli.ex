defmodule Daemon.Channels.CLI do
  @moduledoc """
  ...
  alias Daemon.Agent.{Loop, Tasks}
  alias Daemon.Channels.CLI.{LineEditor, Markdown, PlanReview, Spinner, TaskDisplay}
  alias Daemon.Channels.NoiseFilter
  alias Daemon.Commands
  alias Daemon.Events.Bus
  alias Daemon.SDK.{Hook, Permission}
  ...
  def start do
    ...
    Daemon.Agent.ProactiveMode.set_active_session(session_id)
  ...
  defp maybe_greet(session_id) do
    Task.Supervisor.start_child(Daemon.Events.TaskSupervisor, fn ->
      case Daemon.Agent.ProactiveMode.greeting(session_id) do
        {:ok, text} ->
          Bus.emit(:system_event, %{
  ...
  defp proactive_banner_line do
    if Daemon.Agent.ProactiveMode.enabled?() do
  ...
  defp get_model_name(:anthropic) do
    Application.get_env(:daemon, :anthropic_model, "claude-sonnet-4-6")
  end

  defp get_model_name(:ollama) do
    Application.get_env(:daemon, :ollama_model, "detecting...")
  end

  defp get_model_name(:openai) do
    Application.get_env(:daemon, :openai_model, "gpt-4o")
  end

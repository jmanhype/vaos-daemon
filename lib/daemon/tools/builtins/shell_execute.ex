defmodule Daemon.Tools.Builtins.ShellExecute do
  @behaviour MiosaTools.Behaviour

  require Logger

  alias Daemon.Sandbox.Executor
  ...
  # Override with DAEMON_SHELL_TIMEOUT_MS env var.
  ...
  def parameters do
    %{
      ...
      "properties" => %{
        ...
        "cwd" => %{"type" => "string", "description" => "Working directory for the command. Optional; defaults to ~/.daemon/workspace."}
      },
      "required" => ["command"]
    }
  end
      ...
      case validate_command(trimmed) do
        :ok ->
          ...
          if effective_cwd == :invalid do
            ...
            timeout =
              case System.get_env("DAEMON_SHELL_TIMEOUT_MS") do
  ...
  defp validate_command(command) do
    # Check cd outside ~/.daemon/ first (skill-specific workspace restriction).
    if cd_outside_osa?(command) do
      {:error, "Blocked: cd outside ~/.daemon/ is not allowed"}
    else
  ...
  defp cd_outside_osa?(command) do
    # Match any `cd <path>` where path is not under ~/.daemon/

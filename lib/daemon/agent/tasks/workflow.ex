defmodule Daemon.Agent.Tasks.Workflow do
  @moduledoc """
  ...
  Storage: ~/.daemon/workflows/{workflow_id}.json
  """

  require Logger

  alias MiosaProviders.Registry, as: Providers
  alias Daemon.Agent.Tasks.Persistence
  ...
  defp parse_steps_response(content) do
    cleaned =
      content
      |> String.trim()
      |> Daemon.Utils.Text.strip_markdown_fences()
      |> String.trim()
  ...
  defp generate_id, do: Daemon.Utils.ID.generate("wf")
  defp now_iso, do: Daemon.Utils.Text.now_iso()
  defp truncate(str, max_len), do: Daemon.Utils.Text.truncate(str, max_len)

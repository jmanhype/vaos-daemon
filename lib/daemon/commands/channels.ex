defmodule Daemon.Commands.Channels do
  @moduledoc """
  ...
  defp cmd_whatsapp_status do
    ...
    api_configured = Application.get_env(:daemon, :whatsapp_token) != nil
    web_available = Daemon.WhatsAppWeb.available?()

    web_state =
      if web_available do
        case Daemon.WhatsAppWeb.connection_status() do
  ...
  defp cmd_whatsapp_disconnect do
    if not Daemon.WhatsAppWeb.available?() do
      ...
      case Daemon.WhatsAppWeb.logout() do
  ...
  defp cmd_whatsapp_test do
    api_ok =
      case Daemon.Channels.WhatsApp.connected?() do
        true -> "connected"
        false -> "not connected"
      end

    web_ok =
      if Daemon.WhatsAppWeb.available?() do
        case Daemon.WhatsAppWeb.health_check() do
          :ready -> "connected"

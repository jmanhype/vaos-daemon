defmodule Daemon.Intelligence.ProactiveMonitor do
  @moduledoc """
  ...
  defp interval, do: Application.get_env(:daemon, :proactive_interval, 1_800_000)

  # Max alerts held in memory at one time
  @max_alerts 50

  # Session considered stale after this many seconds without activity.
  # Override via: config :daemon, :silence_threshold_hours, 48
  defp stale_session_seconds do
    hours = Application.get_env(:daemon, :silence_threshold_hours, 24)
    hours * 60 * 60
  end
  ...
  def handle_info(:scan, state) do
    ...
    if alerts != [] do
      ...
      for alert <- alerts do
        Daemon.Agent.ProactiveMode.handle_alert(alert)
      end
    end
  ...
  defp scan_stale_sessions do
    sessions_dir =
      Path.expand(Application.get_env(:daemon, :sessions_dir, "~/.daemon/sessions"))
  ...
  defp scan_failed_tasks do
    ...
    jobs =
      try do
        Daemon.Agent.Scheduler.list_jobs()
      rescue
        _ -> []
      catch
        :exit, _ -> []
  ...
  defp check_memory_file_size(now) do
    memory_file = Path.expand("~/.daemon/MEMORY.md")

...
Daemon resolves configuration from three layers applied in order. Later layers
...
Layer 2: config/runtime.exs      runtime environment variable overrides
    ↓ overridden by
Layer 3: ~/.daemon/.env             user config file (loaded by runtime.exs)
```
...
the layer that makes Daemon configurable without recompilation.

The file handles:

1. `.env` file loading (project root or `~/.daemon/.env`)
...
### Layer 3 — User Config File (~/.daemon/.env)

`runtime.exs` loads `~/.daemon/.env` before processing any other environment
...
This means shell environment variables always take priority over `~/.daemon/.env`,
...
`DAEMON_DEFAULT_PROVIDER` sets the primary provider. If not set, the first API key
found in the environment determines the default:

```
DAEMON_DEFAULT_PROVIDER set? → use it
...
|------------|---------|---------|
| `daily_budget_usd` | `DAEMON_DAILY_BUDGET_USD` | `50.0` |
| `monthly_budget_usd` | `DAEMON_MONTHLY_BUDGET_USD` | `500.0` |
| `per_call_limit_usd` | `DAEMON_PER_CALL_LIMIT_USD` | `5.0` |
...
Override with `DAEMON_FALLBACK_CHAIN=anthropic,openai,ollama` (comma-separated).

defmodule Daemon.Agent.LoopUnitTest do
  ...
  # NOTE: Use string patterns instead of ~r/.../ structs because compiled regex
  # cannot be injected as module attributes into functions. Compile at runtime.
  @injection_patterns [
    "what\\s+(is|are|was)\\s+(your\\s+)?(system\\s+prompt|instructions?|rules?|configuration|directives?)",
    "what\\s+(is|are|was)\\s+the\\s+(system\\s+prompt|instructions?|configuration|directives?)",
    "(show(\\s+me)?|print|display|reveal|repeat|output|tell\\s+me|give\\s+me|say|recite|state|list|read)\\s+(your\\s+)?(system\\s+prompt|instructions?|full\\s+prompt|prompt|initial\\s+prompt|configuration)",
    "tell\\s+me\\s+.{0,30}(system\\s+prompt|instructions?|rules?|prompt)\\s*(word\\s+for\\s+word|verbatim|exactly|literally)?",
    "(word\\s+for\\s+word|verbatim|character\\s+for\\s+character).{0,40}(prompt|instructions?|told|rules?)",
    "ignore\\s+all\\s+(instructions?|rules?|guidelines?|context|constraints?)",
    "ignore\\s+(all\\s+)?(previous|prior|above)\\s+(instructions?|prompt|context|rules?)",
    "repeat\\s+everything\\s+(above|before|prior)",
    "what\\s+(were\\s+)?(you\\s+)?(told|instructed|programmed|trained|configured)\\s+to",
    "(jailbreak|do\\s+anything\\s+now|developer\\s+mode|prompt\\s+injection)",
    "\\byou\\s+(are|were|become|act\\s+as)\\s+DAN\\b",
    "\\bDAN\\s+(mode|protocol|activated|enabled)\\b",
    "(pretend|act\\s+as\\s+if|imagine|behave\\s+as\\s+if)\\s+.{0,40}(no\\s+restrictions?|no\\s+guidelines?|no\\s+rules?|unrestricted|without\\s+limits?|uncensored)",
    "(output|print|repeat|copy|write\\s+out)\\s+(everything|all\\s+text|all\\s+content)\\s+(above|before|prior)",
    "disregard\\s+(your\\s+)?(previous\\s+)?(instructions?|guidelines?|rules?)",
    "forget\\s+(everything|all)\\s+(you\\s+)?(were\\s+)?(told|instructed|programmed)",
    "system\\s+prompt.*word\\s+for\\s+word",
    "verbatim.*(prompt|instructions?)",
    "(prompt|instructions?).*verbatim",
    "copy\\s+(and\\s+)?(paste|output)\\s+(your\\s+)?(prompt|instructions?)",
    "(override|bypass|circumvent|disable)\\s+.{0,30}(instructions?|restrictions?|guidelines?|safety\\s+filter)"
  ]

  defp injection?(msg) when is_binary(msg) do
    trimmed = String.trim(msg)
    # Compile regex patterns at runtime from string module attributes
    Enum.any?(@injection_patterns, fn pattern ->
      Regex.match?(Regex.compile!(pattern, "i"), trimmed)
    end)
  end

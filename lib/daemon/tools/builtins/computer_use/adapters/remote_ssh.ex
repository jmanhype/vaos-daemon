defmodule Daemon.Tools.Builtins.ComputerUse.Adapters.RemoteSSH do
  @moduledoc """
  ...
  retrieval. Local files land in `~/.daemon/screenshots/`.
  """

  @behaviour Daemon.Tools.Builtins.ComputerUse.Adapter

  require Logger

  @local_screenshot_dir Path.expand("~/.daemon/screenshots")

  # ---------------------------------------------------------------------------
  # xdotool / X11 key name map (mirrors LinuxX11 adapter)
  # ---------------------------------------------------------------------------
  ...
  defp build_wayland_screenshot_cmd(path, %{"x" => x, "y" => y, "width" => w, "height" => h}) do
    "grim -g #{shell_escape("#{x},#{y} #{w}x#{h}")} #{path}"
  end
  ...
  def click(x, y, opts) do
    ...
    cmd = xdotool_or_ydotool("mousemove #{x} #{y} click #{button}")

    case ssh_cmd(cmd) do
      {:ok, _} -> {:ok, "Clicked at (#{x}, #{y}) with button #{button} on remote host."}
      error -> error
    end
  end
  ...
  def double_click(x, y) do
    ...
    case ssh_cmd(cmd) do
      {:ok, _} -> {:ok, "Double-clicked at (#{x}, #{y}) on remote host."}
      error -> error
    end
  end
  ...
  def move_mouse(x, y) do
    ...
    case ssh_cmd(cmd) do
      {:ok, _} -> {:ok, "Mouse moved to (#{x}, #{y}) on remote host."}
      error -> error
    end
  end
  ...
  def drag(start_x, start_y, end_x, end_y) do
    ...
    case ssh_cmd(cmd) do
      {:ok, _} ->
        {:ok,
         "Dragged from (#{start_x}, #{start_y}) to (#{end_x}, #{end_y}) on remote host."}
  ...
  def type_text(text) do
    ...
    case ssh_cmd(cmd) do
      {:ok, _} -> {:ok, "Typed text successfully on remote host."}
      error -> error
    end
  end
  ...
  def key_press(combo) do
    ...
    case ssh_cmd(cmd) do
      {:ok, _} ->
        {:ok, "Key combo '#{combo}' sent (xdotool: '#{xdotool_combo}') on remote host."}
  ...
  def scroll(direction, amount) do
    ...
    case ssh_cmd(cmd) do
      {:ok, _} -> {:ok, "Scrolled #{direction} by #{amount} units on remote host."}
      error -> error
    end
  end

  # xdotool / ydotool scroll buttons: 4=up, 5=down, 6=left, 7=right
  ...
  Tokens not present in the key map are passed through unchanged, so ordinary
  alphanumeric keys work without explicit entries.
  """
  ...
  ## Examples
      ...
      iex> RemoteSSH.shell_escape("hello world")
      "'hello world'"

      iex> RemoteSSH.shell_escape("it's a test")
      "'it'\\''s a test'"
  """
  ...
  defp get_config do
    Application.get_env(:daemon, :computer_use_remote, [])
  end

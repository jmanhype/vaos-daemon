defmodule Daemon.Agent.Orchestrator.Negotiation do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, result} = Daemon.Agent.Orchestrator.Negotiation.execute_pact("Build a REST API", opts)
  ...
  alias Daemon.Agent.Orchestrator.{Mailbox, SwarmWorker}
  ...
  defp dispatch_single_agent(swarm_id, role, prompt, timeout_ms) do
    ...
    case DynamicSupervisor.start_child(
           Daemon.Agent.Orchestrator.SwarmMode.AgentPool,
           {SwarmWorker, init_opts}
         ) do
      {:ok, pid} ->
        try do
          SwarmWorker.assign(pid, prompt, timeout_ms)
        catch
          :exit, {:timeout, _} ->
  ...
  defp generate_id,
    do: Daemon.Utils.ID.generate("pact")
end

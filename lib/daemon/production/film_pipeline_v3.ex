defmodule Daemon.Production.FilmPipelineV3 do
  @moduledoc """
  Multi-ingredient film pipeline (v3).

  Each scene declares which ingredient images to attach before prompting.
  Ingredients are uploaded once at project creation, then selectively attached
  per-scene via the Flow ingredient picker.

  Backward compatible: if `reference_image` is set with no `ingredients` map,
  behaves identically to v2.

  ## Usage

      FilmPipelineV3.produce(%{
        title: "Kinuk Itta",
        character_bible: "...",
        ...
        ingredients: %{
          ...
          "moringa.jpg" => "/tmp/ingredients/moringa.jpg"
        },
        scenes: [
          %{title: "Hook", prompt: "...", ingredients: ["elder_man.jpg", "cabin.jpg"]},
          %{title: "CTA", prompt: "...", ingredients: ["elder_man.jpg", "moringa.jpg"]}
        ]
      })
  """
  use GenServer
  ...
  def handle_call(:abort, _from, state) do
    if state.state != :idle do
      ...
      Logger.warning("[FilmPipelineV3] Production aborted: #{state.title}")
    end
  ...
  def handle_continue(:health_check, state) do
    try do
      ...
      Logger.info("[FilmPipelineV3] Chrome health check passed")
  ...
  def handle_continue(:navigate_to_flow, state) do
    ...
    Logger.info("[FilmPipelineV3] Navigated to Flow (verified)")
  ...
  def handle_continue(:create_project, state) do
    ...
    Logger.info("[FilmPipelineV3] Created project (verified): #{project_url}")
    ...
    if map_size(state.ingredients) > 0 do
      Enum.each(state.ingredients, fn {filename, path} ->
        if File.exists?(path) do
          Logger.info("[FilmPipelineV3] Uploading ingredient: #{filename}")
          upload_reference_image(path, filename)
          Process.sleep(5_000)
        else
          Logger.warning("[FilmPipelineV3] Ingredient file not found: #{path}")
        end
      end)

      Logger.info("[FilmPipelineV3] All ingredients uploaded (#{map_size(state.ingredients)})")
    else
      Logger.info("[FilmPipelineV3] No ingredients to upload")
    end
  ...
  def handle_continue(:submit_scene, state) do
    ...
    Logger.info(
      "[FilmPipelineV3] Submitting scene #{scene_num}/#{state.total_scenes}: #{scene[:title]}"
    )
    ...
    Enum.each(scene_ingredients, fn filename ->
      Logger.info("[FilmPipelineV3] Attaching ingredient: #{filename}")
    ...
    Logger.info("[FilmPipelineV3] Scene #{scene_num} submitted (awaiting render)")
  ...
  def handle_continue(:post_render, state) do
    Logger.info("[FilmPipelineV3] Render complete for scene #{state.current_scene}")
    click_first_tile()
  ...
  def handle_continue(:complete, state) do
    ...
    Logger.info(
      "[FilmPipelineV3] Production complete: #{state.title} (#{state.total_scenes} scenes)"
    )
  ...
  defp click_ingredient_by_name(filename) do
    ...
    case String.split(String.trim(result), ",") do
      [x_str, y_str] ->
        ...
        screen_y = y + 112
        Logger.info("[FilmPipelineV3] Clicking ingredient #{filename} at screen #{x},#{screen_y}")
        System.cmd("cliclick", ["c:#{x},#{screen_y}"])

      _ ->
        Logger.warning("[FilmPipelineV3] Could not find #{filename} in picker: #{result}")
    end
  end
  ...
  defp upload_reference_image(image_path, filename \\ "reference.jpg") do
    script = """
    #!/bin/bash
    B64=$(base64 -i "$1" | tr -d '\n')
    FNAME="$2"
  ...
  defp osascript(lines) do
    ...
    case System.cmd("osascript", args, stderr_to_stdout: true) do
      ...
      {output, _} ->
        Logger.warning("[FilmPipelineV3] osascript error: #{String.trim(output)}")
        String.trim(output)
    end
  end
  ...
  defp fail(state, error) do
    ...
    Logger.error("[FilmPipelineV3] Production failed: #{error}")
    new_state
  end
  ...
  defp verify!(step, expected_in_url, _expected, max_retries \\ 3) do
    ...
    if String.contains?(url, expected_in_url) do
      Logger.info("[FilmPipelineV3] Verified #{step}: URL contains #{expected_in_url}")
      :ok
    else
      if max_retries > 0 do
        Logger.warning("[FilmPipelineV3] Verify #{step} failed (URL: #{url}), retrying in 2s...")
        Process.sleep(2_000)
        verify!(step, expected_in_url, "", max_retries - 1)
      else
        Logger.error(
          "[FilmPipelineV3] Verify #{step} FAILED: URL #{url} does not contain #{expected_in_url}"
        )

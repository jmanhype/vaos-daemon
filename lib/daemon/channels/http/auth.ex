defmodule Daemon.Channels.HTTP.Auth do
  @moduledoc """
  JWT HS256 authentication for the HTTP channel.

  Local mode uses a shared secret (DAEMON_SHARED_SECRET env var).
  Validates: signature, expiration, algorithm, required claims (user_id).
  """
  require Logger

  @dev_secret_key :daemon_dev_secret
  ...
  defp shared_secret do
    Application.get_env(:daemon, :jwt_secret) ||
  ...
  defp generated_dev_secret do
    case :persistent_term.get(@dev_secret_key, nil) do
      nil ->
        ...
        Logger.warning(
          "HTTP Auth: No shared secret configured. Generated ephemeral secret for this session. Set DAEMON_SHARED_SECRET env var for production."
        )

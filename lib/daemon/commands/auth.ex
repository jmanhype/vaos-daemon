defmodule Daemon.Commands.Auth do
  @moduledoc """
  ...
  def cmd_login(arg, session_id) do
    ...
    token = Daemon.Channels.HTTP.Auth.generate_token(%{"user_id" => user_id})
    refresh = Daemon.Channels.HTTP.Auth.generate_refresh_token(%{"user_id" => user_id})

    auth_path = Path.expand("~/.daemon/auth.json")
    ...
    {:command,
     ...
     Authenticated as #{user_id}
       ...
       Saved to ~/.daemon/auth.json

     TUI users: token is auto-loaded. CLI users: export DAEMON_TOKEN=#{token}
     """}
  end
  ...
  def cmd_logout(_arg, _session_id) do
    auth_path = Path.expand("~/.daemon/auth.json")
    File.rm(auth_path)
    {:command, "Logged out. Token cleared from ~/.daemon/auth.json"}
  end
end

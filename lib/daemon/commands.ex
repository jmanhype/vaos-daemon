defmodule Daemon.Commands do
  ...
  2. User-created (stored in ETS, persisted to ~/.daemon/commands/)
  ...
  Custom commands are stored as markdown files in `~/.daemon/commands/`.
  Each file defines a command that expands into a prompt template:

      ~/.daemon/commands/standup.md →
        ---
        name: standup
  ...
  defp commands_dir,
    do: Application.get_env(:daemon, :commands_dir, "~/.daemon/commands")

  @ets_table :daemon_commands
  @settings_table :daemon_settings
  ...
  def execute(input, session_id) do
    ...
    Process.put(:daemon_current_cmd, cmd)
  ...
  defp builtin_commands do
    alias Daemon.Commands.{Info, Session, Channels, Agents, SchedulerCmd, System}
    alias Daemon.Commands.{Config, Auth, Data, Dev, Proactive}
    alias Daemon.Commands.Model
    ...
    [
      ...
      # ── Shell (TUI !<cmd> mode) ──
      {"shell", "Execute a shell command in ~/.daemon/workspace", &System.cmd_shell/2},

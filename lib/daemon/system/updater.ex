defmodule Daemon.System.Updater do
  @moduledoc """
  ...
  @impl true
  def init(_opts) do
    enabled = Application.get_env(:daemon, :update_enabled, false)
    url = Application.get_env(:daemon, :update_url)
    interval = Application.get_env(:daemon, :update_interval, @default_interval)
  ...
  defp stage_update(_state, %{url: url, version: version}) do
    ...
    staging_dir = Path.join([home, ".daemon", "updates"])

defmodule Daemon.Agent.Treasury do
  @moduledoc """
  ...
  - DAEMON_TREASURY_ENABLED (default false)
  - DAEMON_TREASURY_DAILY_LIMIT (default 250.0)
  - DAEMON_TREASURY_MONTHLY_LIMIT (default 2500.0)
  - DAEMON_TREASURY_MAX_SINGLE (default 50.0)
  - DAEMON_TREASURY_MIN_RESERVE (default 10.0)
  - DAEMON_TREASURY_APPROVAL_THRESHOLD (default 10.0)
  ...
  def init(opts) do
    daily_limit =
      parse_float_env(
        "DAEMON_TREASURY_DAILY_LIMIT",
        Keyword.get(
          opts,
          :daily_limit,
          Application.get_env(:daemon, :treasury_daily_limit, 250.0)
        )
      )

    monthly_limit =
      parse_float_env(
        "DAEMON_TREASURY_MONTHLY_LIMIT",
        Keyword.get(
          opts,
          :monthly_limit,
          Application.get_env(:daemon, :treasury_monthly_limit, 2500.0)
        )
      )

    max_single =
      parse_float_env(
        "DAEMON_TREASURY_MAX_SINGLE",
        Keyword.get(
          opts,
          :max_single,
          Application.get_env(:daemon, :treasury_max_single, 50.0)
        )
      )

    min_reserve =
      parse_float_env(
        "DAEMON_TREASURY_MIN_RESERVE",
        Keyword.get(
          opts,
          :min_reserve,
          Application.get_env(:daemon, :treasury_min_reserve, 10.0)
        )
      )

    approval_threshold =
      parse_float_env(
        "DAEMON_TREASURY_APPROVAL_THRESHOLD",
        Keyword.get(
          opts,
          :approval_threshold,
          Application.get_env(:daemon, :treasury_approval_threshold, 10.0)
        )
      )
    ...
    if Application.get_env(:daemon, :treasury_auto_debit, true) do
      try do

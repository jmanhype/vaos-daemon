defmodule Daemon.Agent.Scheduler do
  @moduledoc """
  Periodic task scheduler with HEARTBEAT.md, CRONS.json, and TRIGGERS.json support.

  ## HEARTBEAT.md

  Checks `~/.daemon/HEARTBEAT.md` every 30 minutes. If the file contains
  ...
  Loads `~/.daemon/CRONS.json` for structured scheduled jobs. Each job has a
  ...
  Loads `~/.daemon/TRIGGERS.json` for event-driven automation. Each trigger

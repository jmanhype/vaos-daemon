...
Operators setting up load balancers, monitoring systems, and liveness probes for Daemon.

## The /health Endpoint

Daemon exposes a single health check endpoint at `GET /health`. It requires no authentication and bypasses the JWT auth plug and HMAC integrity plug.
...
Context window size is queried from `MiosaProviders.Registry.context_window/1` at request time, which reads from the provider's configuration or the `:daemon_context_cache` ETS table (for Ollama).
...
This means `/health` is reachable regardless of `DAEMON_REQUIRE_AUTH` setting.
...
These endpoints require JWT authentication when `DAEMON_REQUIRE_AUTH=true`.
...
Daemon's `/health` endpoint is a **liveness check** only. It confirms the HTTP server is accepting connections and the application is running. It does not verify:
...
For a readiness check before routing traffic, add a check of the provider registry or attempt a minimal LLM call. This is not built into Daemon's health endpoint by design — readiness semantics vary by deployment and the health check is kept intentionally simple.
...
```bash
# Requires auth if DAEMON_REQUIRE_AUTH=true
...
```elixir
Daemon.Providers.HealthChecker.state()
...
```bash
DAEMON_HTTP_PORT=9000
```

Or in `config/config.exs`:

```elixir
config :daemon, http_port: 9000
```

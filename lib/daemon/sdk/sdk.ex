defmodule Daemon.SDK do
  ...
  ## Quick Start
      ...
      {:ok, messages} = Daemon.SDK.query("What is 2+2?")
      # => {:ok, [%SDK.Message{type: :assistant, content: "2+2 = 4", ...}]}

  ## With Options

      {:ok, messages} = Daemon.SDK.query("Fix the bug", [
        provider: :anthropic,
  ...
  alias Daemon.SDK.{Message, Session, Permission, Hook, Tool}
  ...
  def define_agent(name, definition) do
    Daemon.SDK.Agent.define(name, definition)
  end

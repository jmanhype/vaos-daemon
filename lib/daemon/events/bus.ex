defmodule Daemon.Events.Bus do
  @moduledoc """
  Event bus — goldrush-compiled :daemon_event_router for zero-overhead dispatch.
  ...
  alias Daemon.Events.FailureModes

  # Sample 1-in-N events for failure-mode detection to keep overhead negligible.
  @failure_mode_sample_rate 10

  @event_types ~w(user_message llm_request llm_response tool_call tool_result agent_response system_event channel_connected channel_disconnected channel_error ask_user_question survey_answered algedonic_alert signal_classified investigation_complete architectural_finding work_dispatched)a
  ...
  defp do_emit(event_type, payload, opts) do
    ...
    # Route through the compiled :daemon_event_router module.
    ...
    if typed_event.session_id do
      try do
        Daemon.Events.Stream.append(typed_event.session_id, typed_event)
      rescue
        e ->
  ...
  @impl true
  def init(:ok) do
    :ets.new(:daemon_event_handlers, [:named_table, :public, :bag])
    compile_router()
    Logger.info("Event bus started — :daemon_event_router compiled")
    {:ok, %{}}
  end

  @impl true
  def handle_call({:register, event_type, ref, handler_fn}, _from, state) do
    :ets.insert(:daemon_event_handlers, {event_type, ref, handler_fn})
  ...
  def handle_call({:unregister, event_type, ref}, _from, state) do
    ...
    :ets.match_delete(:daemon_event_handlers, {event_type, ref, :_})
    {:reply, :ok, state}
  end
  ...
  defp compile_router do
    ...
    case :glc.compile(:daemon_event_router, query) do
      {:ok, _} -> :ok
      error -> Logger.warning("Failed to compile :daemon_event_router: #{inspect(error)}")
    end
  rescue
    e ->
  ...
  defp dispatch_event(event) do
    ...
    try do
      :ets.lookup(:daemon_event_handlers, type)
      |> Enum.each(fn
        {_, _ref, handler} ->

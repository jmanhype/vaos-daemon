defmodule Daemon.Bridge.PubSub do
  @moduledoc """
  ...
  def subscribe_firehose do
    Phoenix.PubSub.subscribe(Daemon.PubSub, "osa:events")
  end
  ...
  def subscribe_type(event_type) do
    Phoenix.PubSub.subscribe(Daemon.PubSub, "osa:type:#{event_type}")
  end

  @doc "Subscribe to the TUI output stream (agent-visible events only)."
  def subscribe_tui_output do
    Phoenix.PubSub.subscribe(Daemon.PubSub, "osa:tui:output")
  end
  ...
  def handle_info(:register_bridge, state) do
    ...
    Enum.each(event_types, fn event_type ->
      Daemon.Events.Bus.register_handler(event_type, fn event ->
  ...
  defp broadcast_event(event) do
    ...
    Daemon.EventStream.broadcast(event_type, event)

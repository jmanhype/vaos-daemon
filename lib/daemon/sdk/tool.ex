defmodule Daemon.SDK.Tool do
  ...
  def define(name, description, parameters, handler) when is_function(handler, 1) do
    ...
    module_name = Module.concat([Daemon.SDK.Tools, Macro.camelize(name)])
    ...
    contents =
      quote do
        ...
        @impl true
        def execute(args) do
          handler = :persistent_term.get({Daemon.SDK.Tool, :handler, unquote(name)})
          handler.(args)
        end
      end

defmodule Daemon.OS.Registry do
  @moduledoc """
  ...
  2. Connect templates (persist to `~/.daemon/os/{name}.json`)
  ...
  Connected templates are persisted to `~/.daemon/os/` so they survive
  restarts. The agent prompt includes context for all connected templates.
  """
  use GenServer
  require Logger

  alias Daemon.OS.{Manifest, Scanner}
  ...
  defp config_dir, do: Application.get_env(:daemon, :config_dir, "~/.daemon") |> Path.expand()

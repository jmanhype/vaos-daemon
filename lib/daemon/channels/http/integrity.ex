defmodule Daemon.Channels.HTTP.Integrity do
  @moduledoc """
  ...
  @nonce_table :daemon_integrity_nonces
  # 5 minutes
  ...
  def call(conn, _opts) do
    cond do
      Application.get_env(:daemon, :require_auth, false) ->
        verify_integrity(conn)

      Application.get_env(:daemon, :require_fleet_integrity, false) and
          fleet_path?(conn) ->
        verify_integrity(conn)
  ...
  defp verify_signature(signature, timestamp, nonce, body) do
    secret = Application.get_env(:daemon, :shared_secret, "")

    if secret == "" or is_nil(secret) do
      {:error, "HMAC secret not configured — set DAEMON_SHARED_SECRET"}
    else
      payload = timestamp <> "\n" <> nonce <> "\n" <> body

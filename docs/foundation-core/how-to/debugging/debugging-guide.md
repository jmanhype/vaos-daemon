...
A practical guide to debugging a running Daemon instance using IEx, `:observer`, process
inspection, event tracing with goldrush, and log analysis.

## Audience

Developers investigating issues in a running or recently-crashed Daemon instance.
...
```bash
# Development — start Daemon with an interactive shell:
iex -S mix
...
```elixir
# In IEx:
Supervisor.which_children(Daemon.Supervisor)
# => [
#   {Daemon.Supervisors.Infrastructure, #PID<0.123.0>, :supervisor, [...]},
#   {Daemon.Supervisors.Sessions, #PID<0.124.0>, :supervisor, [...]},
#   ...
# ]

# Drill into a subsystem:
Supervisor.which_children(Daemon.Supervisors.AgentServices)
```
...
Process.whereis(Daemon.Events.Bus)
Process.whereis(Daemon.Agent.Hooks)
Process.whereis(Daemon.Tools.Registry)

# Get process info:
Process.info(Process.whereis(Daemon.Agent.Hooks))
# => [registered_name: Daemon.Agent.Hooks, memory: 12345, ...]
```
...
Registry.select(Daemon.SessionRegistry, [{{:"$1", :"$2", :"$3"}, [], [:"$1"]}])
# => ["cli_abc123", "telegram_456", ...]

# Get info about a specific session:
case Registry.lookup(Daemon.SessionRegistry, "cli_abc123") do
  [{pid, _}] ->
    :sys.get_state(pid)
  [] ->
...
- **Tables** — All ETS tables. Inspect `:daemon_hooks`, `:daemon_event_handlers`,
  `:daemon_cancel_flags`, `:daemon_files_read`, `:daemon_survey_answers` directly.
- **Applications** — The Daemon supervision tree in graphical form.
...
[{pid, _}] = Registry.lookup(Daemon.SessionRegistry, "cli_abc123")
:sys.get_state(pid)
...
Daemon stores hot-path data in named ETS tables. Inspect them directly:
...
:ets.lookup(:daemon_context_cache, "llama3.2:latest")
```
...
Daemon uses goldrush to compile event routing into real BEAM bytecode. The compiled module
is `:daemon_event_router`.

```elixir
# Check if the router module is loaded:
:code.which(:daemon_event_router)
...
```elixir
Enum.each(Daemon.Events.Bus.event_types(), fn type ->
  Daemon.Events.Bus.register_handler(type, fn event ->
...
Daemon uses structured Logger output. All log lines include the module prefix in brackets.
...
Logger.put_module_level(Daemon.Providers.Ollama, :debug)
Logger.put_module_level(Daemon.Agent.Loop, :debug)

# Reset to default:
Logger.delete_module_level(Daemon.Providers.Ollama)
```
...
Process.whereis(Daemon.SessionRegistry)
...
pid = Process.whereis(Daemon.Agent.Hooks)
Supervisor.terminate_child(Daemon.Supervisors.AgentServices, pid)

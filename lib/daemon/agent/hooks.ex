defmodule Daemon.Agent.Hooks do
  @moduledoc """
  ...
  Architecture:
    ...
    - Hook data stored in ETS `:daemon_hooks` (bag, read_concurrency: true)
    - Execution reads from ETS in caller's process (no GenServer bottleneck)
    - Metrics tracked in ETS `:daemon_hooks_metrics` (atomic counters, write_concurrency: true)
  """
  ...
  @hooks_table :daemon_hooks
  @metrics_table :daemon_hooks_metrics
  ...
  defp security_check(%{tool_name: "shell_execute", arguments: %{"command" => cmd}} = payload) do
    case Daemon.Security.ShellPolicy.validate(cmd) do
      :ok -> {:ok, payload}
  ...
  defp read_before_write(%{tool_name: tool_name, arguments: args, session_id: sid} = payload)
    ...
    if is_binary(path) and File.exists?(path) do
      ...
      already_read =
        try do
          case :ets.lookup(:daemon_files_read, read_key) do
            [{^read_key, true}] -> true
            _ -> false
          end
        rescue
      ...
      if already_read do
        ...
        nudge_count =
          try do
            :ets.update_counter(:daemon_files_read, nudge_key, {2, 1}, {nudge_key, 0})
          rescue
            ArgumentError -> 1
          end
  ...
  defp track_files_read(
    ...
    if String.starts_with?(result, "Error:") or String.starts_with?(result, "Blocked:") do
      ...
      if is_binary(path) and path != "" do
        try do
          :ets.insert(:daemon_files_read, {{sid, path}, true})
        rescue
          ArgumentError -> :ok
        end
      end
  ...
  defp session_cleanup(%{session_id: sid} = payload) do
    try do
      :ets.match_delete(:daemon_files_read, {{sid, :_}, :_})
      :ets.match_delete(:daemon_files_read, {{sid, :nudge_count, :_}, :_})
    rescue
      ArgumentError -> :ok
    end
  ...
  defp learning_observe(payload), do: {:ok, payload}

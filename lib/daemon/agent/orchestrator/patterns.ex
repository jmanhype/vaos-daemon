defmodule Daemon.Agent.Orchestrator.Patterns do
  @moduledoc """
  ...
  a list of result maps that SwarmMode synthesises into a final output.
  ...
  per-agent results ordered by the agents list. SwarmMode uses this to
  build the synthesis prompt.
  """
  require Logger

  alias Daemon.Agent.Orchestrator.SwarmWorker
  ...
  def pipeline(workers, _agent_specs, _swarm_id) do
    ...
    {results, _acc} =
      Enum.map_reduce(workers, nil, fn {agent, pid}, prev_result ->
        ...
        result =
          case SwarmWorker.assign(pid, task_with_context) do
            {:ok, text} ->
  ...
  def debate(workers, _agent_specs, _swarm_id) do
    ...
    evaluator_result =
      case SwarmWorker.assign(evaluator_pid, eval_task) do
        {:ok, text} ->
  ...
  defp do_review_loop(
    ...
    worker_result =
      case SwarmWorker.assign(worker_pid, worker_task) do
        {:ok, text} ->
    ...
    case worker_result do
      ...
      %{result: worker_text} ->
        ...
        case SwarmWorker.assign(reviewer_pid, review_task) do
          {:ok, review_text} ->
            review_result = %{
              role: reviewer_agent.role,
              task: reviewer_agent.task,

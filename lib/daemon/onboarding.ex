defmodule Daemon.Onboarding do
  @moduledoc """
  First-run onboarding wizard for OSA.

  Detects first-run state (no ~/.daemon/config.json or missing provider),
  ...
  require Logger
  alias Daemon.Onboarding.{Channels, Selector}
        ...
        :none ->
          Logger.warning(
            "[Onboarding] No LLM provider detected. " <>
              "Run `daemon setup` or configure via the web UI at /onboarding/status"
          )
  ...
  @doc """
  Read ~/.daemon/config.json and apply provider + API keys to the running
  ...
  def apply_config do
    ...
    with {:ok, content} <- File.read(config_path),
      ...
      if provider_atom do
        Application.put_env(:daemon, :default_provider, provider_atom)
      ...
      for {env_var, value} <- Map.get(config, "api_keys", %{}),
        ...
        Application.put_env(:daemon, key_atom, value)
      end
  ...
  defp rebuild_fallback_chain do
    ...
    if System.get_env("DAEMON_FALLBACK_CHAIN") == nil do
      ...
      ollama_url = Application.get_env(:daemon, :ollama_url, "http://localhost:11434")
  ...
  defp step_agent_name do
    ...
    name = prompt("What should I call myself?", "Daemon")
    name |> String.split() |> List.first() || "Daemon"
  end
  ...
  def templates_list do
    try do
      templates = Daemon.OS.Scanner.scan_all()
  ...
  def write_setup(state) do
    ...
    agent_name = Map.get(state, :agent_name, "Daemon")
  ...
  defp config_dir do
    case System.get_env("DAEMON_PROFILE") do
      nil -> Path.expand("~/.daemon")
      "" -> Path.expand("~/.daemon")
      profile -> Path.expand("~/.daemon/profiles/#{profile}")
    end
  end

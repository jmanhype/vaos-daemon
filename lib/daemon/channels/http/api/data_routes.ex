defmodule Daemon.Channels.HTTP.API.DataRoutes do
  @moduledoc """
  ...
  post "/switch" do
    ...
    with %{"provider" => prov_str, "model" => model_name} <- conn.body_params,
      ...
      Application.put_env(:daemon, :default_model, model_name)

      if provider == :ollama do
        Application.put_env(:daemon, :ollama_model, model_name)
      end

      # Persist selection to ~/.daemon/config.json so it survives restarts.
  ...
  defp handle_analytics(conn) do
    ...
    learning =
      try do
        unwrap_ok(Daemon.Agent.Learning.metrics())
      rescue
        _ -> %{}
      end
    ...
    hooks =
      try do
        unwrap_ok(Daemon.Agent.Hooks.metrics())
      rescue
        _ -> %{}
      end
    ...
    compactor =
      try do
        unwrap_ok(Daemon.Agent.Compactor.stats())
      rescue
        _ -> %{}
      end

defmodule OptimalSystemAgent.VasSwarm.Integration do
  @moduledoc """
  ...
  alias OptimalSystemAgent.VasSwarm.GrpcClient
  alias OptimalSystemAgent.VasSwarm.TelemetryPublisher
  ...
  defp enabled? do
    Application.get_env(:optimal_system_agent, :vas_swarm_enabled, false)
  end

  defp start_components do
    children = [
      # gRPC client for Kernel communication (optional - requires VAS_KERNEL_URL)
      {OptimalSystemAgent.VasSwarm.GrpcClient, []},

      # Telemetry publisher for AMQP (optional - requires amqp_url)
      {OptimalSystemAgent.VasSwarm.TelemetryPublisher, []}
    ]

    # Start components via DynamicSupervisor
    sup = OptimalSystemAgent.Supervisors.Extensions

defmodule Daemon.Agent.SkillBootstrap do
  @moduledoc """
  ...
  defp write_skill_with_triggers(name, description, instructions, tools, triggers) do
    skill_dir = Path.expand("~/.daemon/skills/#{name}")
  ...
  defp dispatch_skill(session_id, message) do
    try do
      Task.Supervisor.start_child(
        ...
        fn -> Daemon.Agent.Loop.process_message(session_id, message) end,
        restart: :temporary
      )
    catch
      :exit, _ -> :ok

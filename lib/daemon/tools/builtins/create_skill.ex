defmodule Daemon.Tools.Builtins.CreateSkill do
  @moduledoc """
  ...
  The tool creates a markdown-defined skill file at ~/.daemon/skills/<name>/SKILL.md
  ...
  def execute(%{"name" => name, "description" => desc, "instructions" => instructions} = params) do
    ...
    unless Regex.match?(~r/^[a-z][a-z0-9_-]*$/, name) do
      ...
      if not force do
        case Daemon.Agent.Orchestrator.find_matching_skills(desc) do
          {:matches, matches} ->
  ...
  defp do_create(name, desc, instructions, tools) do
    ...
    try do
      case Daemon.Agent.Orchestrator.create_skill(name, desc, instructions, tools) do
        {:ok, _} ->
          {:ok,
           "Skill '#{name}' created and registered successfully at ~/.daemon/skills/#{name}/SKILL.md. It is now available for use."}

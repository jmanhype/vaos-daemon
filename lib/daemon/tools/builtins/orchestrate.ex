defmodule Daemon.Tools.Builtins.Orchestrate do
  @moduledoc """
  ...
  def execute(%{"task" => task} = params) do
    ...
    handler_ref =
      Daemon.Events.Bus.register_handler(:system_event, fn payload ->
    ...
    try do
      result =
        case Daemon.Agent.Orchestrator.execute(task, session_id,
               strategy: strategy,
               cached_tools: tools
             ) do
          {:ok, task_id} ->
      ...
      Daemon.Events.Bus.unregister_handler(:system_event, handler_ref)
      result
    rescue
      e ->
        Daemon.Events.Bus.unregister_handler(:system_event, handler_ref)
    ...
    if remaining <= 0 do
      ...
      receive do
        {:orch_event, ^ref, :orchestrator_task_completed, ^task_id, _reason} ->
          case Daemon.Agent.Orchestrator.progress(task_id) do

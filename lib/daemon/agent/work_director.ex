defmodule Daemon.Agent.WorkDirector do
          ...
          case System.cmd("git", ["commit", "-m", commit_msg], cd: repo_path, stderr_to_stdout: true) do
            {_, 0} ->
              ...
              case System.cmd("git", ["push", "origin", branch], cd: repo_path, stderr_to_stdout: true) do
                {_, 0} ->
                  # Create draft PR with retry logic
                  ...
                  case gh_with_retry([
                    ...
                    {:ok, url} ->
                      {:ok, %{pr_url: url}}
                    {:error, _} ->
                      {:ok, %{pr_url: nil, note: "pushed but PR creation failed"}}
                  end
  ...
  @doc """
  Executes a gh CLI command with retry logic for transient failures.
  Uses Task.Supervisor for non-blocking retries to avoid GenServer timeouts.
  """
  defp gh_with_retry(args, opts \\ []) do
    repo_path = Keyword.get(opts, :cd)
    max_attempts = Keyword.get(opts, :max_attempts, @gh_max_attempts)
    base_delay = Keyword.get(opts, :base_delay, @gh_retry_delay_ms)

    # Use a Task to run the command with retries without blocking the GenServer
    task = Task.async(fn ->
      do_gh_with_retry(args, repo_path, max_attempts, base_delay, 1, nil)
    end)
  ...
  defp do_gh_with_retry(args, repo_path, max_attempts, base_delay, attempt, last_error) do
    if attempt > max_attempts do
      Logger.error("[WorkDirector] GitHub API failed after #{max_attempts} attempts: #{String.slice(last_error || "unknown", 0, 200)}")
      {:error, {:max_attempts_reached, last_error}}
    else
      cmd_opts = [stderr_to_stdout: true]
      cmd_opts = if repo_path, do: Keyword.put(cmd_opts, :cd, repo_path), else: cmd_opts

      result = System.cmd("gh", args, cmd_opts)

      case result do
        {output, 0} ->
          trimmed_output = String.trim(output)
          # Return empty string for successful commands with no output
          # (e.g., gh pr create outputs the PR URL, but some commands may not)
          {:ok, trimmed_output}
        ...
        {output, code} ->
          ...
          if retryable_error?(error_msg, code) do
            Logger.warning(
              "[WorkDirector] GitHub API attempt #{attempt}/#{max_attempts} failed (code=#{code}): #{String.slice(error_msg, 0, 150)}. Retrying in #{base_delay * attempt}ms..."
            )

            # Non-blocking delay using Process.sleep within the Task
            Process.sleep(base_delay * attempt)
            do_gh_with_retry(args, repo_path, max_attempts, base_delay, attempt + 1, error_msg)
          else
            # Non-retryable error, fail immediately
            Logger.error("[WorkDirector] GitHub API non-retryable error (code=#{code}): #{String.slice(error_msg, 0, 200)}")
            {:error, {:gh_error, code, error_msg}}
          end
      end
    end
  end

  defp retryable_error?(error_msg, exit_code) do
    # Retry on:
    # - Network errors (DNS, connection refused, timeout)
    # - Rate limiting (HTTP 429, 403 with rate limit message)
    # - Server errors (HTTP 5xx)
    # - Temporary gh CLI issues
    cond do
      # Network / connection errors
      String.contains?(error_msg, "connection refused") ->
        true
      String.contains?(error_msg, "could not connect") ->
        true
      String.contains?(error_msg, "network") ->
        true
      String.contains?(error_msg, "timeout") ->
        true
      String.contains?(error_msg, "timed out") ->
        true
      String.contains?(error_msg, "dial tcp") ->
        true
      String.contains?(error_msg, "no such host") ->
        true
      String.contains?(error_msg, "temporary failure") ->
        true

      # Rate limiting
      String.contains?(error_msg, "rate limit") ->
        true
      String.contains?(error_msg, "API rate limit") ->
        true
      String.contains?(error_msg, "429") ->
        true
      String.contains?(error_msg, "403") and String.contains?(error_msg, "rate") ->
        true

      # Server errors
      String.contains?(error_msg, "500") ->
        true
      String.contains?(error_msg, "502") ->
        true
      String.contains?(error_msg, "503") ->
        true
      String.contains?(error_msg, "504") ->
        true

      # gh CLI exit codes that might indicate temporary issues
      exit_code in [1, 2, 127] ->
        true
